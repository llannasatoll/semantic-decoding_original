...
class EncodingModel:
    ...
    def set_shrinkage(self, alpha):
        ...
        precision = np.linalg.inv(
            self.sigma * (1 - alpha) + np.eye(len(self.sigma)) * alpha
        )
    ...
    def prs(self, stim, trs):
        ...
        with torch.no_grad():
            ...
            diff = (
                torch.matmul(stim, self.weights) - self.resp[trs]
            )  # encoding model residuals
            multi = torch.matmul(
                torch.matmul(diff, self.precision), diff.permute(0, 2, 1)
            )
            return (
                -0.5
                * multi.diagonal(dim1=-2, dim2=-1).sum(dim=1).detach().cpu().numpy()
            )
